...
plot.comparisons = function(plot.name,rep.list,rep.names,agg.years = TRUE,agg.regions=TRUE,sel.basis="AGE",biomass.type = "SSB", palette.cols = c("royalblue3","deepskyblue1","gold","orange1","indianred1","firebrick2","#AC2020"),save.dir,save.name)
{
	if(!missing(save.dir))
	{
		if(missing(save.name))
		{
			...
			g3 = plot.selectivity(rep.list,rep.names,sel.basis,palette.cols,save.dir,save.name=paste0("sel-",tolower(sel.basis),"-",save.name))
		}
	} else {
